...
wo new --project "$d" --title "Cart empty" --type feature --problem P \
   --out "manual non-goal" --from-figma "$fig" >/dev/null
f=$(find "$d/work-orders" -maxdepth 1 -name 'WO-*.md')
...
# enormous work-order.
d2=$(new_project)
wo new --project "$d2" --title "Cart only" --type feature --problem P --out X \
   --from-figma "$fig" --frames 'wf/checkout-cart/*' >/dev/null
f2=$(find "$d2/work-orders" -maxdepth 1 -name 'WO-*.md')
...
run 3 "refuses a --frames glob that matches nothing" \
  wo new --project "$(new_project)" --title T --type feature --problem P --out X \
