...
Three markdown files, two entry trees, four scripts, one rule: **an agent never hand-edits a managed file.**
...
5. `backlog/` when choosing work - `now/` / `next/` / `later/` in full (live work, capped by
